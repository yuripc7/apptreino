...
export function HomeTab() {
  ...
  if (todayType === 'Gym') {
    ...
    subtitle = exercisesCount + ' exercicios';
  } else if (todayType === 'Crossfit') {
    subtitle = 'Foco no condicionamento';
  } else {
    subtitle = 'Recuperacao muscular';
  }
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pt-4 pb-10">
      ...
      <section className="bg-gradient-to-br from-surface-accent to-bg border border-border-color rounded-2xl p-5 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-xl bg-accent/20 flex items-center justify-center text-accent">{todayDetails.icon}</div>
          <div>
      ...
      <section className="bg-surface border border-border-color rounded-2xl p-5">
        ...
        <div className="h-3 bg-bg rounded-full overflow-hidden mb-4 border border-border-color">
          <motion.div className="h-full bg-blue-500" initial={{ width: 0 }} animate={{ width: Math.min((water/waterGoal)*100, 100)+'%' }} transition={{ duration: 0.5 }} />
        </div>
        <div className="flex gap-2">
          <button onClick={() => setWater(w => w+250)} className="flex-1 bg-surface-accent hover:bg-bg border border-border-color text-text-primary py-2 rounded-xl text-sm font-medium transition-colors">+250ml</button>
          <button onClick={() => setWater(w => w+500)} className="flex-1 bg-surface-accent hover:bg-bg border border-border-color text-text-primary py-2 rounded-xl text-sm font-medium transition-colors">+500ml</button>
        </div>
      </section>

      <section className="bg-surface border border-border-color rounded-2xl p-5">
        <h2 className="font-bold text-text-primary mb-4">Habitos Diarios</h2>
        <button onClick={() => setCreatineTaken(!creatineTaken)} className={`w-full flex items-center justify-between p-4 rounded-xl border transition-colors ${creatineTaken ? 'bg-accent/10 border-accent' : 'bg-surface-accent border-border-color'}`}>
